...
Future<List<dynamic>> request({required RoomStatus requestQuery}) async {
  String url;

  if (requestQuery.getParamID() != null) {
    url =
        '${AppState().domain}/api/resource/Room Status/${requestQuery.getParamID()}';
  } else {
    url =
        '${AppState().domain}/api/resource/Room Status?${queryParams(requestQuery.formatRequest())}';
  }
