...
class RoomRequest {
  ...
  Map<String, dynamic> toRequestParams() {
    return {
      ...
      if (limitStart != null && limitStart!.isNotEmpty) 'limit_start': limitStart,
...
Future<List<dynamic>> requestItem({required RoomRequest requestQuery}) async {
  ...
  if (response.statusCode == 200) {
    ...
    throw Exception('System unknown error, status code: ${response.statusCode}');
  }
}

Future<Map<String, dynamic>> requestDetail({required RoomRequest requestQuery}) async {
  String url =
  ...
  if (response.statusCode == 200) {
    final responseBody = json.decode(response.body);
    if (responseBody is Map<String, dynamic> && responseBody.containsKey('docs') && responseBody['docs'] is List) {
